...
<body>
  ...
  <div class="header-controls">
    <button id="cart-icon">🛒 Cart (<span id="cart-count">0</span>)</button>
  ...
  <main>
    ...
    <div class="food-item">
      <img src="https://api.campbellsmeat.com/wp-content/uploads/2022/08/HowtoCookSausagesTheCompleteGuideforBeginners.png" alt="Smokies"/>
      <h3>Beef Sausages</h3>
    ...
    <div class="food-item">
      <img src="https://toquehotels.com/wp-content/uploads/2025/01/KENYAN-BHAJIAS-2.jpg" alt="Bhajia"/>
      <h3>Bhajia</h3>
    ...
    <div class="food-item">
      ...
      <h3><a href="fresh_juice.html" class="food-link">Fresh Juice</a></h3>
    </div>
    ...
    <div class="food-item">
      <img src="https://toplinebites.com/wp-content/uploads/2024/07/african-tea.jpg" alt="African Tea"/>
      <h3>African Tea</h3>
      <p class="price">Ksh.50</p>
  ...
  <aside id="cart-container" aria-label="Shopping Cart">
    <h2>Your Cart</h2>
    <div id="cart-items">
      <p>Your cart is empty.</p><!-- Placeholder text when cart is empty -->
    </div>
...
<script>
  ...
  let cartItems = JSON.parse(localStorage.getItem('ronz_cart')) || [];  // Load cart items into UI
...
// Render the cart on page load
  ...
  toggle.addEventListener('click', (e) => {
    e.stopPropagation();
    nav.classList.toggle('active');// Show/hide nav menu
  });

  document.addEventListener('click', (e) => {
    if (!nav.contains(e.target) && !toggle.contains(e.target)) {
      nav.classList.remove('active');// Hide menu if clicking outside
    }
  });

  cartIcon.addEventListener('click', () => {
    cartContainer.classList.toggle('active');// Toggle cart sidebar
  });

  closeCartBtn.addEventListener('click', () => {
    cartContainer.classList.remove('active');// Close cart sidebar
  });

  function addToCart(item) {
    const existingItem = cartItems.find(i => i.id === item.id);
    if (existingItem) {
      existingItem.quantity++;// Increment quantity if item already in cart
    } else {
      cartItems.push({ ...item, quantity: 1 });// Add new item with quantity
    }
    localStorage.setItem('ronz_cart', JSON.stringify(cartItems));// Save updated cart
    renderCartItems();// Refresh cart UI
  ...
  function renderCartItems() {
    ...
    if (cartItems.length === 0) {
      ...
      checkoutBtn.style.display = 'none';// Hide checkout if empty
    } else {
      ...
      cartItems.forEach(item => {
        ...
        totalCost += item.price * item.quantity;// Add to total
      });

       // Show total
      const totalDiv = document.createElement('div');
      totalDiv.classList.add('cart-total');
      totalDiv.innerHTML = `<p><strong>Total: Ksh.${totalCost.toFixed(2)}</strong></p>`;
  ...
  checkoutBtn.addEventListener('click', () => {
    ...
    .then(data => {
      if (data.success) {
        localStorage.setItem('last_order_total', total.toFixed(2));// Store order
        localStorage.removeItem('ronz_cart');// Clear cart
        cartItems = [];
        renderCartItems();// Refresh UI
        cartContainer.classList.remove('active');// Hide cart
        window.location.href = 'thankyou.html';// Redirect to thank you
      } else {
  ...
  window.addToCart = addToCart;// Make addToCart globally accessible
</script>

  <footer class="footer">
    <p>© 2025 Ron'z Pizza. All rights reserved.</p>
  </footer>
